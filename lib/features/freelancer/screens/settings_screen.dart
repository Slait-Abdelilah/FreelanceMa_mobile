...
class _FreelancerSettingsScreenState extends State<FreelancerSettingsScreen> {
  ...
  Future<void> _togglePrivacy(String key, bool value) async {
    // Optimistic update
    final prev = _settings?[key] as bool? ?? false;
    setState(() => _settings?[key] = value);
    try {
      await _svc.updatePrivacy({
        'showOnlineStatus': _settings?['showOnlineStatus'] as bool? ?? false,
        'publicProfile':    _settings?['publicProfile']    as bool? ?? false,
        'showEarnings':     _settings?['showEarnings']     as bool? ?? false,
      });
    } catch (_) {
      if (mounted) {
                ...
                children: [
                  ...
                  _Section(title: 'COMPTE', children: [
                    ...
                    _Tile(
                      ...
                      onTap: () => _showSheet(context, _ChangePasswordSheet()),
                    ),
                    _Tile(
                  ...
                  // ── Confidentialité ──────────────────────────────────────
                  _Section(title: 'CONFIDENTIALITÉ', children: [
                    _SwitchTile(
                      icon: Icons.public_outlined,
                      label: 'Profil public',
                      value: _settings?['publicProfile'] as bool? ?? false,
                      onChanged: (v) => _togglePrivacy('publicProfile', v),
                    ),
                    _SwitchTile(
                      icon: Icons.bar_chart_outlined,
                      label: 'Afficher mes gains',
                      value: _settings?['showEarnings'] as bool? ?? false,
                      onChanged: (v) => _togglePrivacy('showEarnings', v),
                    ),
                    _SwitchTile(
                      icon: Icons.radio_button_checked_outlined,
                      label: 'Statut en ligne visible',
                      value: _settings?['showOnlineStatus'] as bool? ?? false,
                      onChanged: (v) => _togglePrivacy('showOnlineStatus', v),
                    ),
                  ]),
